...
function Start()
    ...
    scene_api.BindEvent("NPC/skilladv", "wumingshangu.TalkGirl")
    scene_api.BindEvent("NPC/merchant", "wumingshangu.TalkMerchant")
...
function TalkGirl()
    ...
    local ret = ShowSelectPanel(1, "<color=yellow>武学10级后可进阶\n</color>", {"武学进阶", "武学升级","<color=grey>武学学习</color>","离开"})

    if ret == 0 then
        if Jyx2.SkillMgr:JudgeCanAdvSkill() then

            local selectSkillId = Jyx2.UIHelper:ShowCanAdvSkillSelectPanel(girlId)
            ...
            if selectSkillId ~= -1 then
                ...
                if Jyx2.Util:IsPlayerHaveItem(skillAdvItemId) == true then
                    ...
                    --开始进阶
                    local state,message = Jyx2.SkillMgr:SkillAdv(selectSkillId)
...
function TalkBeichou()
    ...
    elseif(flag == 2) then
        ...
        local ret = 3
        if(level % 2 == 1) then
            ret = ShowSelectPanel(roleId, "汝欲神兵、秘笈，还是队友？\n选择后将<color=red>自动存档</color>，不容反悔。", {item.Name, book.Name, teamMate.Name,"随机大礼包","再想想"})
        else
            ::label_retry::
            ret = ShowSelectPanel(roleId, "汝欲神兵、秘笈？\n选择后将<color=red>自动存档</color>，不容反悔。", {item.Name, book.Name, "<color=grey>(暂不可选队友)</color>","随机大礼包", "再想想"})
            if(ret == 2) then
                goto label_retry
            end
        end
